 ...
 * @since 0.3.0
 */
var WPCV_CGI_Manual_Sync = WPCV_CGI_Manual_Sync || {};
...
( function( $ ) {
	 ...
	 * @since 0.3.0
	 */
	WPCV_CGI_Manual_Sync.settings = new function() {
		...
		this.init_localisation = function() {
			if ( 'undefined' !== typeof WPCV_CGI_Manual_Sync_Vars ) {
				me.localisation = WPCV_CGI_Manual_Sync_Vars.localisation;
			}
		};
		...
		this.init_settings = function() {
			if ( 'undefined' !== typeof WPCV_CGI_Manual_Sync_Vars ) {
				me.settings = WPCV_CGI_Manual_Sync_Vars.settings;
			}
		};
	...
	function CVGRP_ProgressBar( options ) {
		...
		// Assign labels.
		me.label_init = WPCV_CGI_Manual_Sync.settings.get_localisation( options.key, 'total' );
		me.label_current = WPCV_CGI_Manual_Sync.settings.get_localisation( options.key, 'current' );
		me.label_complete = WPCV_CGI_Manual_Sync.settings.get_localisation( options.key, 'complete' );
		me.label_done = WPCV_CGI_Manual_Sync.settings.get_localisation( 'common', 'done' );
		...
		this.send = function() {
			...
			// URL to post to.
			url = WPCV_CGI_Manual_Sync.settings.get_setting( 'ajax_url' );
	 ...
	 * @since 0.3.0
	 */
	WPCV_CGI_Manual_Sync.progress_bar = new function() {
		...
		this.setup = function() {
			...
			data = WPCV_CGI_Manual_Sync.settings.get_setting( 'wp_to_civicrm' );
			...
			data = WPCV_CGI_Manual_Sync.settings.get_setting( 'civicrm_to_wp' );
	...
	// Init settings.
	WPCV_CGI_Manual_Sync.settings.init();

	// Init Progress Bar.
	WPCV_CGI_Manual_Sync.progress_bar.init();
...
jQuery(document).ready(function($) {
	...
	WPCV_CGI_Manual_Sync.settings.dom_ready();

	// The DOM is loaded now.
	WPCV_CGI_Manual_Sync.progress_bar.dom_ready();
